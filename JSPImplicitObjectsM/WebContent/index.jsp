...
<%@ page errorPage="handle-page.jsp" %>
<!DOCTYPE html>
<html>
<head>
...
<body>
<%
	//JSP Implicit Objects - request, response, session,config,application,page,pageContext
	String responseCheck = request.getParameter("office");
	if(responseCheck!=null)
	{
		response.setStatus(response.SC_MOVED_TEMPORARILY);
		response.setHeader("Location", "response-redirect.jsp?office=" + responseCheck); // Equivalent of response.sendRedirect()
	}
	String errorCheck = request.getParameter("error");
    if (errorCheck != null ) {
             int x = 0;
             if (x == 0)
                     throw new RuntimeException("Exception has been raised"); // Wantedly throwing an exception here.
    }
%>
